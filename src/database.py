"""
database.py  –  Database session helpers and atomic operations.

Re-exports session management from the new infrastructure layer.
Keeps backward-compatible atomic operations for the old service layer.
New code should use container.get_container() from container.py.
"""
...
def ensure_account_exists(acc_no: str, name: str = "", balance: float = 0.0):
    ...
    from infrastructure.persistence import AccountModel
...
#  These are kept for the old services.py and test compatibility.
#  New code should use application/services.py directly via the container.
# ═══════════════════════════════════════════════════════════════════════════════

class AtomicTransferResult:
    def __init__(self, success: bool, sender_balance: float = 0.0,
                 receiver_balance: float = 0.0, error_message: str = ""):
...
def atomic_transfer(
    sender_acc_no: str, receiver_acc_no: str, amount: float,
    category: str = "General", sender_name: str = "", receiver_name: str = "",
...
def atomic_apply_interest(acc_no: str, interest_amount: float) -> bool:
    ...
    if interest_amount > 0:
        ...
        txn.create(Transaction(
            txn_id=__import__('utils').generate_transaction_id(),
            account_number=acc_no,
